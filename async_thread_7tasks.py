...
async def PlayAsync(taskName,event_for_wait, event_for_set,stopEvent):
    while True:
        ...
        asyncio.create_task(PlayAsyncWithTrhead(taskName,stopEvent))
...
async def task(cancel_event):
    ...
    leftDeckTask = asyncio.create_task(PlayAsync("Left Desk Task",leftDeckEvent, rightDeckEvent,cancel_event))
    rightDeckTask = asyncio.create_task(PlayAsync("Right Desk Task",rightDeckEvent, leftDeckEvent,cancel_event))
